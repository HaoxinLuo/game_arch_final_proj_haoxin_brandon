...
bool separating_axis_test(const ga_shape* a, const ga_mat4f& transform_a, const ga_shape* b, const ga_mat4f& transform_b, ga_collision_info* info)
{
	...
	for (uint32_t i = 0; i < 3; ++i)
	{
		...
		{
			axes.push_back(ga_vec3f_cross(oobb_a._half_vectors[i], oobb_b._half_vectors[j]).normal());
		}
	...
	for (uint32_t i = 0; i < axes.size(); ++i) 
	{
		...
		ga_vec3f sum_a_projection = ga_vec3f(ga_vec3f::zero_vector());
		ga_vec3f sum_b_projection = ga_vec3f(ga_vec3f::zero_vector());
		for (uint32_t i = 0; i < 3; ++i) 
		{
			ga_vec3f a_half_vector = oobb_a._half_vectors[i];
			ga_vec3f b_half_vector = oobb_b._half_vectors[i];
			sum_a_projection += a_half_vector.project_onto_abs(axis);
			sum_b_projection += b_half_vector.project_onto_abs(axis);
		}
		
		ga_vec3f a_center_projection = oobb_a._center.project_onto(axis);
		ga_vec3f b_center_projection = oobb_b._center.project_onto(axis);
		float distance_between_centers = a_center_projection.dist(b_center_projection);
		float sum_of_radii = (sum_a_projection + sum_b_projection).mag();
		...
		if (sum_of_radii - distance_between_centers < min_penetration)
		{
			min_penetration = sum_of_radii - distance_between_centers;
