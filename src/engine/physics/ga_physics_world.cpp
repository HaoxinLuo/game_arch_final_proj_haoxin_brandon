...
void ga_physics_world::step_linear_dynamics(ga_frame_params* params, ga_rigid_body* body)
{
	...
	ga_vec3f x1 = ga_vec3f::zero_vector();
	ga_vec3f v1 = ga_vec3f(body->_velocity);
	
	ga_vec3f x2 = x1 + v1.scale_result(dt / 2.0f);
	ga_vec3f v2 = v1 + a.scale_result(dt / 2.0f);

	ga_vec3f x3 = x1 + v2.scale_result(dt / 2.0f);
	ga_vec3f v3 = v1 + a.scale_result(dt / 2.0f);

	ga_vec3f x4 = x1 + v3.scale_result(dt);
	ga_vec3f v4 = v1 + a.scale_result(dt);

	ga_vec3f dx = (v1 + (v2 + v3).scale_result(2.0f) + v4).scale_result(dt / 6.0f);	
	ga_vec3f dv = (a + (a + a).scale_result(2.0f) + a).scale_result(dt / 6.0f);
	body->_velocity += dv;
	body->_transform.translate(dx);
}
...
void ga_physics_world::resolve_collision(ga_rigid_body* body_a, ga_rigid_body* body_b, ga_collision_info* info)
{
	...
	if (!(body_a->_flags & k_static) && !(body_b->_flags & k_static))
	{
		float body_a_velocity_projection = body_a->_velocity.dot(info->_normal);
		float body_b_velocity_projection = body_b->_velocity.dot(info->_normal);
		float sum_inverse_masses = (1.0f / body_a->_mass) + (1.0f / body_b->_mass);
		ga_vec3f impulse = info->_normal.scale_result(((cor_average + 1) * (body_b_velocity_projection - body_a_velocity_projection)) / sum_inverse_masses);
		body_a->_velocity += impulse.scale_result(1.0f / body_a->_mass);
		body_b->_velocity -= impulse.scale_result(1.0f / body_b->_mass);
	}
	else
	{
		ga_rigid_body* moveable_body = (body_b->_flags & k_static) ? body_a : body_b;
		ga_vec3f impulse = info->_normal.scale_result(-(cor_average + 1) * moveable_body->_mass * moveable_body->_velocity.dot(info->_normal));
		moveable_body->_velocity += impulse.scale_result(1.0f / moveable_body->_mass);
	}
}
