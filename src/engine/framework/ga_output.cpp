...
void ga_output::draw_dynamic(const std::vector<ga_dynamic_drawcall>& drawcalls, const ga_mat4f& view_proj)
{
	for (auto& d : drawcalls)
	{
		...
		GLuint polycolor;
		if (!d._colorarray.empty())
		{
			glGenBuffers(1, &polycolor);
			glBindBuffer(GL_ARRAY_BUFFER, polycolor);
			glBufferData(GL_ARRAY_BUFFER, sizeof(ga_vec3f) * d._colorarray.size(), &d._colorarray[0], GL_STREAM_DRAW);
